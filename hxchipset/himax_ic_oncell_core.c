...
static int himax_mcu_chip_self_test(void)
{
	...
	if (info_data != NULL && info_data16 != NULL) {
		...
		g_core_fp.fp_diag_register_set(on_pfw_op->data_dc_set[0], 0x01);
...
static int himax_mcu_read_ic_trigger_type(void)
{
	...
	g_core_fp.fp_register_read(on_pfw_op->addr_trigger_addr,
			DATA_LEN_4, tmp_data, 0);
		...
		uint8_t rw_ctrl) /*r:0, w:1*/
{
	...
	if (!rw_ctrl) {
		...
		I("%s: Read SFR! data[0] = 0x%02X\n", __func__, data[0]);
		I("%s: Read SFR! data[1] = 0x%02X\n", __func__, data[1]);
		I("%s: Read SFR! data[2] = 0x%02X\n", __func__, data[2]);
		I("%s: Read SFR! data[3] = 0x%02X\n", __func__, data[3]);
	} else {
