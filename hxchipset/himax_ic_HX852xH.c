...
/*  Himax Android Driver Sample Code for HX852xH chipset
 *
...
#include "himax_ic_HX852xH.h"
#include "himax_modular.h"

static void hx852xH_chip_init(void)
{
...
static void himax_hx852xH_func_re_init(void)
{
	kp_g_core_fp->fp_chip_init = hx852xH_chip_init;
}

static void himax_hx852xH_reg_re_init(void)
{
}

static bool hx852xH_chip_detect(void)
{
	...
	himax_hx852xH_reg_re_init();

	himax_hx852xH_func_re_init();
	...
	for (i = 0; i < 5; i++) {
		...
		I("%s: Read driver IC ID = %X,%X,%X\n", __func__,
			tmp_data[3], tmp_data[2], tmp_data[1]); /*85,33,00*/

		if ((tmp_data[3] == 0x85) && (tmp_data[2] == 0x33)) {
...
DECLARE(HX_MOD_KSYM_HX852xH);

static int himax_hx852xH_probe(void)
{
	I("%s: Enter\n", __func__);
	himax_add_chip_dt(hx852xH_chip_detect);
	return 0;
}

static int himax_hx852xH_remove(void)
{
	free_chip_dt_table();
	return 0;
}

static int __init himax_hx852xH_init(void)
{
	int ret = 0;

	I("%s\n", __func__);
	ret = himax_hx852xH_probe();
	return 0;
}

static void __exit himax_hx852xH_exit(void)
{
	himax_hx852xH_remove();
}

module_init(himax_hx852xH_init);
module_exit(himax_hx852xH_exit);

MODULE_DESCRIPTION("HIMAX HX852xH touch driver");
MODULE_LICENSE("GPL");
